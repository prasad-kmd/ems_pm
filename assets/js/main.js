...
const pmbsweb_config = {
    ...
    // Cursor Following
    cursorFollow: '.pmbsweb-socials-list a, .pmbsweb-gallery-nav',

    // Links Exception
    linksException: [
        '.comment-reply-link',
        '.shadowcore-lightbox-link',
        '.pmbsweb-lightbox-link',
        '.pmbsweb-noanim-link'
    ]
}
...
jQuery(function ($) {
    ...
    const core = new ST_Core( pmbsweb_config );
    ...
    function MainMenuInit() {
        template.menu = new Pmbsweb_Menu( core );
    }

    // --- PHOTOSWITE LIGHTBOX --- //
    function initPSWP() {
        if ( jQuery('.pmbsweb-lightbox-link').length ) {
            if ( typeof PhotoSwipe === 'function' ) {
                        ...
                        <div class="pswp__scroll-wrap">\
                            ...
                            <div class="pswp__ui pswp__ui--hidden">\
                                <div class="pswp__top-bar">\
                                    ...
                                    \
                                    <button class="pswp__button pswp__button--close pmbsweb-pswp-close" title="Close (Esc)">\
                                        <i></i>\
                                    </button>\
                                    \
                                ...
                                \
                                <button class="pswp__button pmbsweb-pswp-nav pmbsweb-pswp-prev pswp__button--arrow--left" title="Previous (arrow left)"></button>\
                                <button class="pswp__button pmbsweb-pswp-nav pmbsweb-pswp-next pswp__button--arrow--right" title="Next (arrow right)"></button>\
                                \
                ...
                // Build PSWP Gallery
                jQuery('.pmbsweb-lightbox-link').each(function() {
                    ...
                    if ( $this.attr('data-type') && $this.attr( 'data-type' ) !== 'image' ) {
                        ...
                        if ( 'video' === $this.attr('data-type') ) {
                            this_item.html = '\
                            <div class="pmbsweb-pswp-media--video" data-src="' + $this.attr('href') + '">\
                            ...
                            this_item.html = '\
                            <div class="pmbsweb-pswp-media--iframe" data-player-src="'+ this_link +'" data-src="' + $this.attr('href') + '" data-type="' + this_type + '">\
                ...
                core.$dom.body.on('click', '.pmbsweb-pswp-image-wrap', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
                ...
                jQuery(document).on('click', '.pmbsweb-lightbox-link', function(e) {
                    e.preventDefault();
                    if (typeof PhotoSwipe === 'function') {
                        let $this = jQuery(this),
                        ...
                        template.pswp.lightbox.listen('resize', function() {
                            if ( jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--video').length ) {
                                jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                            if ( jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--iframe').length ) {
                                jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                        });
                        ...
                        template.pswp.lightbox.listen('beforeChange', function() {
                            ...
                            if ( jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--video').length ) {
                                jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--video').each(function() {
                            ...
                            if ( jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--iframe').length ) {
                                jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.pmbsweb-pswp-media--iframe').each(function() {
                ...
                console.error('Pmbsweb Error: PSWP script not found.');
            }
        }
    }
    ...
    function HeroSections() {
        // Masked Blocks
        if ( jQuery('.pmbsweb-masked-block').length ) {
            jQuery('.pmbsweb-masked-block').each(function() {
                new Pmbsweb_Masked(this);
            });
        }
    }
    ...
    function BlocksHighlight() {
        if ( jQuery('.pmbsweb-backlight').length ) {
            template.cursor_pos = {
                x: 0,
                y: 0
            }
            jQuery(document).on('mousemove', function(e) {
                ...
                jQuery('.pmbsweb-backlight').css({
                    '--mouse-x': `${template.cursor_pos.x}px`,
                    '--mouse-y': `${template.cursor_pos.y}px`,
                });
            });

            if ( jQuery('.pmbsweb-block').length ) {
                jQuery('.pmbsweb-block').each(function() {
    ...
    // --- MARQUEE ELEMENT --- //
    function Marquee() {
        if ( jQuery('.pmbsweb-marquee').length ) {
            jQuery('.pmbsweb-marquee').each(function() {
                new Pmbsweb_Marquee(this);
            });
        }
    }
        ...
        if ( jQuery('.swiper').length ) {
            ...
            const init_swiper = function() {
                ...
                jQuery('.swiper').each(function() {
                    ...
                    if ( core.$dom.body.hasClass('pmbsweb-fullscreen-page') ) {
                        options.mousewheel = true;
                    }
                    // CAROUSEL
                    if ( $this.hasClass('pmbsweb-carousel') ) {
                    ...
                    // SLIDER
                    if ( $this.hasClass('pmbsweb-slider') ) {
                        ...
                        // Slider Content
                        if ( $this.find('.pmbsweb-slide-content').length ) {
                            ...
                            $this.children('.swiper-wrapper').children('.swiper-slide').each(function(i) {
                                ...
                                if ( $slide.children('.pmbsweb-slide-content').length ) {
                                    const $slide_content = $slide.children('.pmbsweb-slide-content');
                                    ...
                                    $media.wrap('<div class="pmbsweb-slider-media"/>');
                                    ...
                                    $slide.children('.pmbsweb-slider-media').css('clip-path', `url(#${(content_mask_id + i)})`);
                    ...
                    // Pagination
                    if ( $this.children('.pmbsweb-dots').length ) {
                        options.pagination = {
                            el: '.pmbsweb-dots',
                            dynamicBullets: true,
                    ...
                    // Slider Navigation
                    if ( $this.parent().children('.pmbsweb-slider-nav').length ) {
                        let $this_nav = $this.parent().children('.pmbsweb-slider-nav');
                        options.navigation = {
                            prevEl: $this_nav.children('.pmbsweb-slider-prev')[0],
                            nextEl: $this_nav.children('.pmbsweb-slider-next')[0],
                        }
            ...
            if ( typeof Swiper === 'function' ) {
                ...
                console.error('Pmbsweb Error: Swiper Library not found.')
            }
        }
    }
    ...
    function CircleLinks() {
        if ( jQuery('.pmbsweb-circle-path-link:not(.is-init)').length ) {
            jQuery('.pmbsweb-circle-path-link:not(.is-init)').each(function() {
                let $this = jQuery(this),
    ...
    function LinksEvents() {
        jQuery('.pmbsweb-back-to-listing').on('click', function() {
            window.localStorage.setItem('pmbsweb_return_from', this.getAttribute('data-id'));
        });
        if ( jQuery('.pmbsweb-albums-listing').length && window.localStorage.getItem('pmbsweb_return_from') != false ) {
            jQuery(window).trigger( 'pmbsweb_set_album', window.localStorage.getItem('pmbsweb_return_from') );
            window.localStorage.setItem('pmbsweb_return_from', 0);
        } else {
            window.localStorage.setItem('pmbsweb_return_from', 0);
        }
    }
    ...
    function InfiniteList() {
        if ( jQuery('.pmbsweb-infinite-list-wrap:not(.is-init)').length ) {
            jQuery('.pmbsweb-infinite-list-wrap:not(.is-init)').each(function() {
                this.classList.add('is-init');
                core.updateRequired.push( new Pmbsweb_Infinite_List(this) );
            })
        }
    }
    ...
    // --- COUNTERS --- //
    function Counters() {
        if ( jQuery('.pmbsweb-counter:not(.is-init)').length ) {
            jQuery('.pmbsweb-counter:not(.is-init)').each(function() {
                let $this = jQuery(this);
                this.classList.add('is-init');
                this.counter_count = function() {
                    let $counter = $this.children('.pmbsweb-counter-number');
    ...
    function ListWithPreview() {
        if ( jQuery('.pmbsweb-list-with-preview').length ) {
            jQuery('.pmbsweb-list-with-preview').each(function() {
                new Pmbsweb_LWP(this);
            });
        }
    }
    ...
    function BeforeAfter() {
        if ( jQuery('.pmbsweb-before-after:not(.is-init)').length ) {
            jQuery('.pmbsweb-before-after:not(.is-init)').each(function() {
                jQuery(this).addClass('is-init');
                core.updateRequired.push( new Pmbsweb_Before_After(jQuery(this)) );
            });
        }
    }
    ...
    // --- TOGGLES --- //
    function Toggles() {
        if ( jQuery('.pmbsweb-toggles-item:not(.is-init)').length ) {
            jQuery('.pmbsweb-toggles-item:not(.is-init)').each(function() {
                let $this = jQuery(this);
                $this.addClass('is-init');
                $this.children('.pmbsweb-toggles-item--title').append('<i class="pmbsweb-icon pmbsweb-icon-arrow-down"/>');
                $this.children('.pmbsweb-toggles-item--content').slideUp(1);
                $this.on('click', function() {
                    jQuery(this).toggleClass('is-active');
                    $this.children('.pmbsweb-toggles-item--content').slideToggle(300);
                });
                $this.on('click', '.pmbsweb-toggles-item--content', function(e) {
                    e.stopPropagation();
                    e.preventDefault();
                });
            });
    ...
    function ScrollTrigger() {
        ...
        if ( jQuery('.pmbsweb-inline-ribbon:not(.is-init)').length ) {
            jQuery('.pmbsweb-inline-ribbon:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Sticky Elements
        if ( jQuery('.pmbsweb-sticky-block:not(.is-init)').length ) {
            jQuery('.pmbsweb-sticky-block:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Expand on Scroll
        if ( jQuery('.pmbsweb-expand-on-scroll:not(.is-init)').length ) {
            jQuery('.pmbsweb-expand-on-scroll:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Text Mask Fill
        if ( jQuery('.pmbsweb-text-fill-mask:not(.is-init)').length ) {
            jQuery('.pmbsweb-text-fill-mask:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Parallax Media
        if ( jQuery('.pmbsweb-parallax-media:not(.is-init)').length ) {
            jQuery('.pmbsweb-parallax-media:not(.is-init)').each(function() {
                let $this = jQuery(this);
    ...
    function ContactForm() {
        if ( jQuery('.pmbsweb-contact-form').length ) {
            // The code related to the contact form is in the file: js/contact_form.js
            if ( typeof Pmbsweb_Contact_Form === 'function' ) {
                Pmbsweb_Contact_Form();
            } else {
                console.error('Pmbsweb Error: Contact form script not found.')
            }
        }
    }
    ...
    function InitAnimation() {
        ...
        template.checkURL = function( el ) {
            ...
            if ( this_href.indexOf('javascript') === 0 ) {
                ...
                jQuery(pmbsweb_config.linksException).each(function() {
                    if ( $el.is(this) ) {
                        result = false;
                    }
                });
    ...
    function Layout() {
        ...
        if ( core.$dom.body.hasClass('pmbsweb-fullscreen-page') ) {
            ...
            core.$dom.main.removeClass('pmbsweb-min-content');
            ...
            if ( core.$dom.main.height() < minHeight ) {
                core.$dom.main.addClass('pmbsweb-min-content');
